...
export class ProgrammingLandingComponent implements OnInit {
 ...
 value: any;
  ...
  studentPrgmPage(event: any,i:any){
    ...
    if (clickedBtn.startsWith('q')) {
      const index = parseInt(clickedBtn.substring(1)) - 1; // Extract index from the button id
  
      selectedQuestionId = window.btoa(this.questions[index].Questionid);
      selectedQuestion = window.btoa(this.questions[index].Question);
      selectedQuestionHeading = window.btoa(this.questions[index].QuestionHeading);
    }
